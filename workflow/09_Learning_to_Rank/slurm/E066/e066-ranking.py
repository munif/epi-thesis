...
PREFIX = f"batch-mode-{MODEL_MODE}-{DROPOUT_RATE}"
...
# Initialize model
if MODEL_MODE == "reg":
    model = BC.BinaryClassifier(input_size=INPUT_SIZE).to(device)
elif MODEL_MODE == "do":
    model = BC.BinaryClassifierDropOut(input_size=INPUT_SIZE, dropout_rate=DROPOUT_RATE).to(device)
