...
NUM_EPOCHS = 100
START_EPOCH = 0
NUM_ITEMS = 1_000_000     # 100_000, 1_000_000
BATCH_SIZE = 4096       # 128, 512, 1024, 2048, 4096
DROPOUT_RATE = 0.5
...
CHECKPOINT_NAME = os.path.join(WORKING_DIR, 'output', 'model', MODE, f'{MODEL_MODE}-{DROPOUT_RATE}-checkpoint-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.pt')
...
def plot_metrics(train_losses, val_losses, train_accuracies, val_accuracies, num_items):
    ...
    img_name = f"{MODEL_MODE}-{DROPOUT_RATE}-ranking-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.png"
...
if MODEL_MODE == "reg":
    ...
    model = BinaryClassifierDropOutL1L2(input_size=INPUT_SIZE, dropout_rate=DROPOUT_RATE).to(device)
...
test_file_name = os.path.join(WORKING_DIR, 'output', 'test', MODE, f"{MODEL_MODE}-{DROPOUT_RATE}-test-results-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.txt")
with open(test_file_name, 'w', newline = '') as text_file:
    text_file.write(test_output)

# Training metrics
file_name = f"{MODEL_MODE}-{DROPOUT_RATE}-train-validation-metrics-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.csv"
...
# Test output
test_name = f"{MODEL_MODE}-{DROPOUT_RATE}-test-metrics-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.csv"
...
img_name = f"{MODEL_MODE}-{DROPOUT_RATE}-roc-{NUM_ITEMS}-{NUM_EPOCHS}-{BATCH_SIZE}.png"
