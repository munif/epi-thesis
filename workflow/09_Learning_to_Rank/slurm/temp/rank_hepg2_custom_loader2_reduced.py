...
def plot_metrics(train_losses, val_losses, train_accuracies, val_accuracies, num_items):
    ...
    img_name = f"ranking-{num_items}-reduced.png"
...
# Loading dataset
X = pl.read_parquet(os.path.join(DATASET_DIR, 'X_reduced.parquet'))
y = pl.read_parquet(os.path.join(DATASET_DIR, 'y_reduced.parquet'))
gene_id_features = pl.read_parquet(os.path.join(DATASET_DIR, 'gene_id_features_reduced.parquet'))
...
model_filename = os.path.join(WORKING_DIR, 'output', 'model', f'checkpoint-{num_items}.pt')
early_stopping = EarlyStopping(patience=10, verbose=True, filename=model_filename)
...
# Training metrics
file_name = f"train-validation-metrics-{num_items}-reduced.csv"
...
# Test output
test_name = f"test-metrics-{num_items}-reduced.csv"
...
img_name = f"roc-{num_items}-reduced.png"
