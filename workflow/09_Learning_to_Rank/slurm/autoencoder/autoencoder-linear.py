...
# Initialize model
n_bottleneck = 128
ae_model = LinearAutoEncoder(n_inputs, n_bottleneck).to(device)
...
for epoch in range(n_epochs):
    ...
    ae_model.eval()
    with torch.no_grad():
        test_outputs = ae_model(X_test_tensor)
...
torch.save(ae_model.state_dict(), 'autoencoder-linear.pth')

ae_model = LinearAutoEncoder(n_inputs, n_bottleneck).to(device)
ae_model.load_state_dict(torch.load('autoencoder-linear.pth'))

X_col_tensor = torch.tensor(X_col_2d, dtype=torch.float32).to(device)

ae_model.eval()
with torch.no_grad():
    X_encoded = ae_model.encode(X_col_tensor)
