...
#     https://colab.research.google.com/drive/1uwHcFjnjVCGUsp0KJyFP_FF7q119OPpa
# """
...
OUTPUT_PATH  = Path('/group/pmc021/amunif/epi-thesis/workflow/18_Pairwise Ranking HepG2 REMC/output/')
...
def antisymmetry_score(model, X_test_tensor, device):
    """
    For each pair (gene1, gene2), also predict (gene2, gene1) by swapping
    the two feature halves. A perfectly antisymmetric model always flips:
        pred(A,B) + pred(B,A) == 1
    ...
    # Diagnose near-boundary pairs
    near_boundary  = ((prob_orig - 0.5).abs() < 0.1).sum().item()
    violations     = (~antisym_mask).sum().item()

    # Check if violations cluster near the boundary
    violation_probs = (prob_orig - 0.5).abs()[~antisym_mask]

    print(f"  Antisymmetry score:      {antisym_fraction:.4f}")
    ...
    if len(violation_probs) > 0:
        print(f"  Avg |prob-0.5| at violation: {violation_probs.mean():.4f}  "
              f"(near 0 = boundary issue, larger = real failure)")
...
for number in range(START, END):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # --- Introduce Antisymmetry loss: for every pair, pred(A,B) + pred(B,A) should = 1 ---
            ...
            ANTISYM_LAMBDA = 0.5   # Tune this if necessary
    ...
    antisym = antisymmetry_score(model, X_test_all, device)
    ...
    test_output = (
        ...
        f"Antisymmetry score:  {antisym:.4f}  "
...
"""Some notes:
- The twin-subnet architecture achieved 88.3% antisymmetry on test pairs. All violations occurred at sigmoid output = 0.5, where the score difference between genes was effectively zero and the binary threshold assigns the label arbitrarily. Among pairs where the model expressed any confidence (|p − 0.5| ≥ 0.05), antisymmetry was 100%, confirming that the architecture enforces the property as proven theoretically.
"""